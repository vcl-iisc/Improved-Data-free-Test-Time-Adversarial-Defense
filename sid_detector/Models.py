...
class DualCIFAR4(nn.Module):
    ...
    def __init__(self, C_Number=3, num_class=10):
        ...
        #     in_c = 4
        # self.wave = wave
        # self.DWT = DWTForward(J=1, wave = self.wave, mode='symmetric')
        #### PDmodel
    ...
    def plugdata(self, x, Yl, Yh, mode):
        device = x.get_device()
        if mode == 'append':
            output = torch.zeros(x.shape[0], x.shape[1] * 4, Yl[:, :, :].shape[2], Yl[:, :, :].shape[3]).to(device)
            output = output
            output[:, 0:3, :] = Yl[:, :, :]
            output[:, 3:6, :] = Yh[0][:, 0, :, :]
            output[:, 6:9, :] = Yh[0][:, 1, :, :]
            ...
            output = torch.zeros(x.shape[0], 4, Yl[:, :, :].shape[2], Yl[:, :, :].shape[3]).to(device)
...
class FDVgg(nn.Module):
    ...
    def plugdata(self, x, Yl, Yh, mode):
        ...
        if mode == 'append':
            ...
            output[:, 6:9, :] = Yh[0][:, 1, :, :]
            output[:, 9:12, :] = Yh[0][:, 2, :, :]
            output = output.reshape(x.shape[0], x.shape[1] * 4, Yl[:, :, :].shape[2], Yl[:, :, :].shape[3]).to(device)
        elif mode == 'avg':
