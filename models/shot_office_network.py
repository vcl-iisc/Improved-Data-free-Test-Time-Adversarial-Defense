...
class feat_classifier(nn.Module):
    def __init__(self, class_num, bottleneck_dim=256, type="linear", p=0.00):
        ...
        self.dad_dropout = nn.Dropout(p=p)

    def forward(self, x):
        x = self.dad_dropout(x)
        x = self.fc(x)
        return x

class feat_classifier_two(nn.Module):
    def __init__(self, class_num, input_dim, bottleneck_dim=256, p=0.00):
